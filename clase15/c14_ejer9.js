...
*/
//----------COPIO DE CLASE, NO LO PUDE RESOLVER---------------------------------------------------------------


let modelosAutos = ["Ford", "Renault", "Citroen", "Chevrolet", "Ford", "Peugot", "Citroen"];

function ultimaAparicionModeloAuto (modelo) {

    let posicion = -1;   //usamos -1 para indicar qe "todavia no se encontró nada". Si al final sigue -1 es xq el modelo no está en la lista.

    for (let i = 0; i < modelosAutos.length; i++){ // recorremos el array

        if (modelosAutos[i] === modelo) {
            posicion = i + 1; //guardamos la posición actual (i + 1 para que sea legible por humanos) 
                            //No detenemos el blucle (break). Si aparece otro "Ford" + adelante ...sigue explicación mmmm.
        }

    }
    // evaluamos si la variable posición cambió su valor inicial
    if ( posicion !== -1) {
...
ultimaAparicionModeloAuto("Audi");